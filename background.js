...
/**
 * Sets up event listeners to ensure that the appropriate icon is set based on the current theme
 * when Thunderbird starts or when the extension is installed.
 */
messenger.runtime.onStartup.addListener(updateIconBasedOnTheme);
messenger.runtime.onInstalled.addListener(updateIconBasedOnTheme);
messenger.theme.onUpdated.addListener(updateIconBasedOnTheme);

/**
 * Updates the browser action icon based on the current theme (light or dark).
 *
 * This function checks if the theme is dark or light, and then sets the appropriate icon for the
 * extension's toolbar button.
 */
function updateIconBasedOnTheme() {
    messenger.theme.getCurrent().then(async (theme) => {
        console_log("Update Icon");

        if (theme.properties.color_scheme === "dark") {
            console_log("Dark Icon");
            // Set icon for dark mode
            await messenger.messageDisplayAction.setIcon({
                path: {
                    "48": "icons/email_48x48_inverted.png",
                    "64": "icons/email_inverted.png"
                }
            });
        } else {
            console_log("Light Icon");
            // Set icon for light mode
            await messenger.messageDisplayAction.setIcon({
                path: {
                    "48": "icons/email_48x48.png",
                    "64": "icons/email.png"
                }
            });
        }
