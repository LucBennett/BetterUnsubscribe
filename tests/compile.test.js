...
describe('Build script consistency tests', () => {
    ...
    test.each(buildScripts)(
        ...
        async (script) => {
            ...
            const outputFiles = fs.readdirSync(buildOutputDir).filter(file => file.endsWith('.xpi') || file.endsWith('.zip'));
            if (outputFiles.length === 0) {
                throw new Error(`No output archive found in ${buildOutputDir}`);
            }
            const outputFile = outputFiles[0];
            const outputZip = path.join(buildOutputDir, outputFile);
            const outputExtracted = path.join(tempDir, script.name.replace(/ /g, '_'));
