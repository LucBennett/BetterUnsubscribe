...
const {searchUnsub, UnsubMethod, UnsubMail, UnsubWeb, UnsubPost} = require('../src/background.js');
...
describe('searchUnsub', () => {
    ...
    test.each(emailMessages)(
        'Test message %# against db',
        async (message) => {
...
function getChoice(obj) {
    ...
    if (typeof obj.choice === 'string') {
        choices = [obj.choice];
    } else if (typeof obj.choice === 'object' && Array.isArray(obj.choice.choices)) {
