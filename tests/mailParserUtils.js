...
async function parseMessage(text) {
    ...
    const messageHeader = {
        ...
        size: parsedEmail.size || Buffer.byteLength(
    ...
    function createMessagePart(part, partName = '1') {
        ...
        } else if (part.contentType === 'text/plain' || part.contentType === 'text/html' || part.contentType === 'multipart/mixed') {
            // It's the email body
        ...
        if (part.childNodes && part.childNodes.length > 0) {
            ...
            part.childNodes.forEach((child, index) => {
                messagePart.parts.push(createMessagePart(child, `${partName}.${index + 1}`));
            });
        }
